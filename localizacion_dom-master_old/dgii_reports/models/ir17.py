#  Copyright (c) 2024 - Infinity Services SRL. (https://www.infinityservices.com.do) <info@infinityservices.com.do>
...
class DgiiReport(models.Model):
    ...
    ret_award = fields.Monetary(compute="_compute_ir7_fields",
                                string="Premios (Ley 253-17)")
    ret_title_transfer = fields.Monetary(
        compute="_compute_ir7_fields", string="Transferencia de Titulo y Propiedades")
    ret_dividends = fields.Monetary(
        compute="_compute_ir7_fields", string="Dividendos (Ley 253-17)")
    ...
    ret_remittances = fields.Monetary(
        compute="_compute_ir7_fields", string="Remesas al exterior (Ley 253-12)")
    ret_special_remittances = fields.Monetary(
        compute="_compute_ir7_fields", string="Remesas acuerdos especiales")
    ...
    ret_others_ret = fields.Monetary(
        compute="_compute_ir7_fields", string="Otras retenciones Norma 07-2007")
    ...
    ret_total = fields.Monetary(compute="_compute_ir7_fields",
                                string="Total otras retenciones")

    ret_rent_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Alquileres base")
    ret_service_honoraries_base = fields.Monetary(
        compute="_compute_ir7_fields",
        string="Honorarios por Servicios Independientes base"
    )
    ret_award_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Premios (Ley 253-17) base")
    ret_title_transfer_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Transferencia de Titulo y Propiedades base")
    ret_dividends_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Dividendos (Ley 253-17) base")
    ...
    ret_remittances_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Remesas al exterior (Ley 253-12) base")
    ret_special_remittances_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Remesas acuerdos especiales base")
    ret_local_supplier_base = fields.Monetary(
        compute="_compute_ir7_fields",
        string="Pagos a proveedores del estado (Ley 253-12) base"
    )
    ret_phone_set_base = fields.Monetary(
        string="Juegos Telefónicos (Norma 08-2011) base")
    ret_capital_earning_base = fields.Monetary(
        string="Ganancia de capital (Norma 07-2011) base")
    ...
    ret_others_ret_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Otras retenciones Norma 07-2007 base")
    ...
    ret_finance_entity_physical_base = fields.Monetary(
        compute="_compute_ir7_fields",
        string="Intereses pagados por entidades financieras a personas fisicas (Ley 253-12) Base"
    )

    ret_total_base = fields.Monetary(
        compute="_compute_ir7_fields", string="Total otras retenciones base")
    ...
    def _compute_ir7_fields(self):
        for rec in self:
            ...
            ir7_line_ids = self.env["dgii.reports.ir17.line"].search(
            ...
            rec.ret_service_honoraries_base = abs(
                data_base['ret_service_honoraries_base'])
            ...
            rec.ret_physical_person10_base = abs(
                data_base['ret_physical_person10_base'])
            ...
            rec.ret_special_remittances_base = abs(
                data_base['ret_special_remittances_base'])
            ...
            rec.ret_finance_entity_legal_base = abs(
                data_base['ret_finance_entity_legal_base'])
            rec.ret_finance_entity_physical_base = abs(
                data_base['ret_finance_entity_physical_base'])
    ...
    def _get_move_ir17(self, states, types):
        """ Obtiene facturas filtradas por estado y tipo """

        # Verificar si `name` está vacío o malformateado
        if not self.name or "/" not in self.name:
            raise ValueError(f"Formato incorrecto en el nombre del reporte: {self.name}")

        try:
            month, year = self.name.split("/")
            last_day = calendar.monthrange(int(year), int(month))[1]
        except ValueError:
            raise ValueError(f"El campo 'name' no tiene un formato válido: {self.name}")
    ...
    def _get_ir17_type(self, line, type_isr, tax_amount):
        ...
        elif type_isr == '04' and tax_amount == 2:
            return 'ret_others_rent2'
        ...
        for rec in self:
            ...
            for inv in invoice_ids:
                ...
                for line in inv.line_ids.filtered(
                    lambda x: x.account_id.isr_retention_type
                ):
            ...
            for m in move_ids:
                for line in m.line_ids.filtered(
                    lambda l: l.account_id.account_fiscal_type
